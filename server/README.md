...
**同一个 Worker 还负责网站的三个文字页**：`carelogue.ca/privacy`、`/terms` 与
`/support`（ASC 的 Support URL），内容直接从 `docs/legal/` 与 `docs/web/` 的
Markdown 渲染（打包时作为 Text 模块进包），配色跟 app 一致。一次部署，两个
域名——路由在 `wrangler.toml` 里分，代码按路径分发。
...
GET  /terms       → 使用条款网页（同上）
GET  /support     → 支持页（ASC Support URL，由 docs/web/support.md 渲染）
...
8. **自定义域**（等 carelogue.ca 接进 Cloudflare 之后）：取消 `wrangler.toml` 里四条
   `[[routes]]` 的注释，重新 deploy。四条各司其职：
   ...
   | `carelogue.ca/support` | 支持页（ASC 的 Support URL 指向这里）|
...
```sh
cd server
npm test              # 62 个用例（解释链路 + prompt 组装 + 内部通道 + 网站文字页），不需要网络
...
改 `docs/legal/*.md`（或支持页 `docs/web/support.md`）就行，重新 `npm run deploy`
即可生效——网页、仓库文本、App Store
审核用的那份是同一份，不会各自漂移。渲染器只支持文档里实际用到的 Markdown 子集
（标题 / 段落 / 加粗 / 链接 / 列表 / 表格 / 分隔线）；加新语法要同时补 `src/markdown.js`

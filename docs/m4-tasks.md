...
**⚠️ 注（2026-09-23）**：老板与太太使用**不同 Apple ID**（同家庭组）——两台手机无法互验私有库同步；双机验证需用**同一 Apple ID 的两台设备**（如老板 iPhone + 任意可登录其 ID 的设备）；太太设备单独验证她自己的库。跨 Apple ID 数据共享见 `m5-notes.md`（重要缺口）。

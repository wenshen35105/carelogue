...
- **选型定案（2026-09-22）**：AI 服务商 = **DeepInfra**（DeepSeek-V4-Flash 开源版，$0.09/$0.18 每 M tokens，默认不落盘不训练；备选 Fireworks）；Server = **Cloudflare Workers**（serverless，仅 CPU 计费，$5/月档）；**订阅校验无状态**（StoreKit Transaction JWS 验签，server 零用户库、无账号系统）；定价 ~$4.99/月

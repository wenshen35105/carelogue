...
## 四点五、备选讨论：自建 server 转发 / 同步？

**问过**（见下日期）：能不能用我们的 Cloudflare server 转发数据做跨 ID 共享，会不会更靠谱？

**结论：不做。** 三点：

1. **撞红线**：`server 无状态转发、不存 health data` 已写进隐私政策 / 隐私标签 / 全部对外叙事——改为中转或存储 = 换产品定位（数据到我们手上，合规与责任全面升级），与"不想担责"相反。
2. **工程量**：自建 = 配对机制 + 同步协议 + 冲突 + 加密 + 推送 + 存储 + 持续运维，**3–6 周起步**且是长期负担。CKShare 的"自研"只有桥；自建则是"桥 + 数据承载"全自研——面更大，不是更小。
3. **"靠谱"比较**：CKShare 路里数据承载是 Apple 官方云（与现状一致，非新增）；真正更"官方"的替代是路线 A（Core Data 官方共享 API），轮不到自建 server。server 登场时机 = 做跨平台（backlog 远目标）时再评估。

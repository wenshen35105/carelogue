        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("解释时，Carelogue 会在这台设备上提取报告文字，仅把文字发送给 DeepSeek 用于生成白话说明；不发送图像，不存储、不用于训练。你可以随时在设置中撤回同意。")
                        .font(.body)
                        .foregroundStyle(Theme.inkPrimary)
                        .lineSpacing(4)
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    point(icon: "lock",
                          ...
                          detail: String(localized: "只有文字经 HTTPS 加密通道发送给 DeepSeek；Carelogue 没有自己的服务器。"))
